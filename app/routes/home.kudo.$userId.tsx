...
import { KudoInputForm } from "~/utils/types.server";
...
export const action: ActionFunction = async ({ request }) => {
  ...
  const backgroundColor = form.get('backgroundColor') as Color;
  const textColor = form.get('textColor') as Color;
  const emoji = form.get('emoji') as Emoji;
  const recipientId = form.get('recipientId');

  try {
    const kudoMsg = KudoInputForm.parse({ message, recipientId });
    const newKudo = {
      ...kudoMsg, userId, style: {
        backgroundColor, textColor, emoji,
      }
    }
    await createKudo(newKudo);
    return redirect('/home');
  } catch (err) {
...
export default function KudoModal() {
  ...
  return (
    <Modal isOpen={true} className="w-2/3 p-10">
      <div className="text-xs font-semibold text-center tracking-widetext-center tracking-wide text-red-500 w-full mb-2">
        {formError}
      </div>
      <form method="POST">
